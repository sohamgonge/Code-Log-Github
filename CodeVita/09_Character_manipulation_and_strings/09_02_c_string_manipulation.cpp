...
int main()
{
    // std::strlen : Fidn the length of a string
    // real arrays and those dedcayed into pointers
    ...
    */
    ...
    // std::strcmp - signature : int strcmp( const char *Ihs, const char *rhs);
    // Returns negative value if Ihs appears before rhs in lexicographical order,
    // Zero if Ihs and rhs compare equal
    // and Positive value if Ihs appears after rhs in lexicographical order.
    ...
    // string_data3 = "Alabama"; ILLEGAL
    // string_data4 = "Alabama"; ILLEGAL

    // Print out the coparison
    std::cout << "std::strcmp (" << string_data1 << "," << string_data2 << ") : " << std::strcmp(string_data1,string_data2) <<std::endl;
    */
    ...
    */
    ...
    // we sue std::strchr to find all the chracters one by one
    
    /*
    const char *const str {"Try not. Do, or do not. There is no try."};
    // Can make thsi a const pointer to prevent users from making it point somewhere else
    char target = 'T';
    ...
    while ((result = std::strchr(result, target)) != nullptr)
    {
        ...
        // Increment result, otherwise we'll fidn target at the same location
        ++result;
        ++iterations; 
    }
    ...
    */
    ...
    if(output){
        ...
        // +1 because we want to start printing past the character foudn by std::strrchr/
